...
initializeProgram pData = ProgramState  makeRegisters (makeMemory pData) makeScreen 0x0 programStartAddr [] 0x0 0x0 makeKeyState False
...
fontData = V.fromList $ zero ++ one ++ two ++ three ++ four ++ five ++ six ++ seven ++ eight ++ nine ++ a ++ b ++ c ++ d ++ e ++ f
  where
